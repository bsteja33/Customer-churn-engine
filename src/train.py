...
# ``mlflow`` and ``datasets`` are training-only dependencies
# (requirements-dev.txt). They are imported lazily inside the functions
# that need them so the inference runtime can import this module
# without pulling the training stack.


from src.feature_engineering import encode_dataset_frame
...
def _encode_features(dataframe: pl.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    """One-hot encode categoricals and split off the target column.

    Delegates the encoding to the shared ``encode_dataset_frame`` core
    in ``src.feature_engineering`` so training and inference use the
    exact same transformation (train/serve parity is structural).
    Returns pandas objects, matching the frame the model is fit on.
    """
    features_x = encode_dataset_frame(dataframe.drop("Churn").to_pandas())
    target_y = dataframe["Churn"].to_pandas()
    return features_x, target_y


def engineer_features(dataframe: pl.DataFrame, config: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    logger.info("FEATURE_ENGINEERING_DONE: %d features, %d samples", len(features_x.columns), len(features_x))
    return features_x, target_y
