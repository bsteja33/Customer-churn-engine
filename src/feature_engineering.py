...
This module is the single authoritative implementation used by BOTH
the training pipeline (``src/train.py``) and the inference path
(``api/app.py`` / ``src/predict.py``). Training and serving call the
same :func:`encode_dataset_frame` core, so the train/serve feature
parity is structural rather than behavioral: any change to encoding
immediately applies to both paths and is pinned by regression tests
(``tests/test_src.py::TestTrainServeParity``).
"""
...
def encode_dataset_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Transform a dataset-named DataFrame into the model-ready matrix.

    This is the shared encoding core for training and inference. Input
    columns use the dataset naming convention (``Gender``,
    ``Senior Citizen``, ``Tenure in Months``, ...); callers holding
    API-style snake_case fields should rename first via
    :func:`col_map` (see :func:`engineer_features_inference`).

    Steps: blank ``Total Charges`` coerced to ``0.0``; ``Senior
    Citizen`` stringified so both one-hot columns always exist;
    ``"None"``/``""`` sentinels mapped to null for the null-bearing
    categoricals; explicit one-hot encoding against the training
    vocabulary; regex column sanitization and numeric coercion so
    LightGBM never sees object/bool dtypes.
    """
...
def engineer_features_inference(df: pd.DataFrame) -> pd.DataFrame:
    """Transform raw API-style input into the model-ready matrix.

    Renames snake_case API fields to dataset columns via
    :func:`col_map`, then delegates to :func:`encode_dataset_frame`.
    Used by the API and the CLI predict path.
    """
    mapping = col_map()
    rename = {k: v for k, v in mapping.items() if k in df.columns}
    return encode_dataset_frame(df.rename(columns=rename))
