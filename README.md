...
flowchart LR
  ...
  API -- "JSON" --> Next
  Form -- "POST /api/generate_retention_script" --> LLM[LLM provider<br/>dynamic tier routing]
...
# exercises the full form → results flow with mocked network routes)
...
- **CORS:** the default tuple includes the production Vercel
  origin (`https://customerchurnengine.vercel.app`) plus the
