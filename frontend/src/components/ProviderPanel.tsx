...
export function ProviderPanel({ open, onClose }: ProviderPanelProps) {
  // Mounting the dialog only while open gives it a fresh ``draftKey``
  // snapshot on every open without synchronizing state in an effect.
  if (!open) return null;
  return <ProviderDialog onClose={onClose} />;
}
...
function ProviderDialog({ onClose }: { onClose: () => void }) {
  ...
  // Seeded once at mount from the persisted value; the input stays a
  // local draft until Save commits it back to the store.
  const [draftKey, setDraftKey] = useState(
    useProviderStore.getState().key
  );
  const [catalogLoaded, setCatalogLoaded] = useState(false);
  const [catalogError, setCatalogError] = useState<string | null>(null);

  useEffect(() => {
    // Fetch the backend model catalog once per dialog mount; key edits
    // don't need to re-hit the network. The cancelled flag prevents a
    // late response from touching state after an early close.
    let cancelled = false;
    fetchModelCatalog()
      .then(() => {
      ...
      .catch((e: unknown) => {
        if (!cancelled) {
          setCatalogError(
            e instanceof Error ? e.message : "Catalog unavailable"
          );
        }
      });
    return () => {
      ...
      >
        ...
        <div className="flex flex-col gap-5 p-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <span className="text-[10px] text-white/30 font-mono leading-relaxed">
              The backend routes each request to a suitable model from
              the provider&apos;s live catalog - a medium-intelligence
              model for standard insights and the highest-intelligence
              model available for deep analysis. Specific model ids are
              never pinned in the app, so the routing keeps working as
              the provider evolves its lineup.
            </span>
            {catalogLoaded && (
