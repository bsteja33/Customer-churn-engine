...
// Module-level singleton: refcounted subscribers share one interval.
let subscribers = 0;
...
// The `_set` parameter is required by zustand's signature but unused -
// all mutations go through `useHealthStore.setState` in pollOnce above
// (the `_` prefix exempts it from the unused-vars rule).
